...
echo "[INFO] [GIT] This is the Git folder linker file."
...
echo "[INFO] [GIT] directory is : $currpath"

#changing the path variables to the static variables. 
echo "[INFO] [GIT] converting the scripts. "
count=0
for j in "${relpath[@]}"
do
	path_array[$count]="$currpath$j"
	#echo "[DEBUG] [GIT] processed object path: ${path_array[ $count ] } "
	((count++))
done

#adding the paths to the location of the parent. 
echo "[INFO] [GIT] Adding the shell scripts. "
for j in "${path_array[@]}"
do
	echo "[INFO] [GIT] Adding the file: $j"
	if [ -f $j ]; then
	    	. $j
		echo "[INFO] [GIT] The following file was linked: $j"
	fi
done
